...
fn test_stress() {
    ...
    fn validate(allocator: &RawAlloc<ToyHeap>, allocated_size: usize, freed_size: usize) {
        ...
        log::info!(
            ...
            allocated_size,
            freed_size,
            allocator.grower.size.load(Ordering::Relaxed),
            validity,
            stats,
        );
        ...
        let found_heap_size = &allocator.grower.size.load(Ordering::Relaxed);
        let found_freed = stats.size.load(Ordering::Relaxed);
